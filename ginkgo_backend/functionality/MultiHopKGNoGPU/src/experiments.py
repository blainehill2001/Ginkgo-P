...
class Args:
    def __init__(self, run_single_pred_e1, run_single_pred_r):
        self.data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data/umls")
        self.model = "point.rs.conve"
        self.group_examples_by_query="False"
        self.use_action_space_bucketing="True"

        self.bandwidth=400
        self.entity_dim=200
        self.relation_dim=200
        self.history_dim=200
        self.history_num_layers=3
        self.num_rollouts=20
        self.num_rollout_steps=2
        ...
        self.baseline="n/a"
        self.grad_norm=5
        self.emb_dropout_rate=0.1
        self.ff_dropout_rate=0
        self.action_dropout_rate=0.95
        self.action_dropout_anneal_interval=1000
        self.reward_shaping_threshold=0
        self.beta=0.05
        self.relation_only="False"
        self.beam_size=1

        # pre-trained embedding based model checkpoints
        self.distmult_state_dict_path=os.path.join(os.path.dirname(os.path.dirname(__file__)), "model/umls-distmult-xavier-200-200-0.003-0.3-0.1/model_best.tar")
        self.complex_state_dict_path=os.path.join(os.path.dirname(os.path.dirname(__file__)), "model/umls-complex-xavier-200-200-0.003-0.3-0.1/model_best.tar")
        self.conve_state_dict_path=os.path.join(os.path.dirname(os.path.dirname(__file__)), "model/umls-conve-RV-xavier-200-200-0.003-32-3-0.3-0.3-0.2-0.1/model_best.tar")

        self.num_paths_per_entity=-1
        self.margin=-1

        self.inference=1
        self.save_beam_search_paths=True
        self.run_single_pred_e1 = run_single_pred_e1
        self.run_single_pred_r = run_single_pred_r
        ...
        self.run_ablation_studies = False
        self.model_root_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'model')
        self.add_reversed_training_edges = False
        self.train_entire_graph = False
        self.xavier_initialization = False
        self.uniform_entity_initialization = False
        self.num_graph_convolution_layers = 0
        self.rnn_dropout_rate = 0.0
        self.relation_only_in_path = False
        self.emb_2D_d1 = 10
        self.emb_2D_d2 = 20
        self.num_out_channels = 32
        self.kernel_size = 3
        self.hidden_dropout_rate = 0.3
        self.feat_dropout_rate = 0.2
        self.start_epoch = 0
        self.adam_beta1 = 0.9
        self.adam_beta2 = 0.999
        self.run_analysis = False
        self.gamma = 1
        self.action_dropout_anneal_factor=0.95
        self.mu = 1.0
        self.checkpoint_path = None
        self.compute_map = False
        self.eval_by_relation_type = False
        self.eval_by_seen_queries = False
        self.mask_test_false_negatives = False

    def update_run_single_pred(self, x1, x2):
        self.run_single_pred_e1, self.run_single_pred_r = x1, x2
...
def inference(lf):
    ...
    elif args.run_single_pred_e1 and args.run_single_pred_r:
        dev_path = os.path.join(args.data_dir, 'dev.triples')
        test_path = os.path.join(args.data_dir, 'test.triples')
        test_data = data_utils.load_triples(
            test_path, entity_index_path, relation_index_path, seen_entities=seen_entities, verbose=False, run_single_pred_e1=args.run_single_pred_e1, run_single_pred_r=args.run_single_pred_r)

        #print('Test set performance:')
        pred_json = lf.forward_ginkgo(test_data, verbose=args.save_beam_search_paths)
        print(pred_json)
        return pred_json
    else:
        dev_path = os.path.join(args.data_dir, 'dev.triples')
...
def run_ablation_studies(args):
    ...
    #print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(to_m_ratio, to_m_mrrs['ours'][''], to_m_mrrs['-rs'][''], rel_change(to_m_mrrs, '-rs', ''), to_m_mrrs['-ad'][''], rel_change(to_m_mrrs, '-ad', ''),to_1_ratio, to_1_mrrs['ours'][''], to_1_mrrs['-rs'][''], rel_change(to_1_mrrs, '-rs', ''), to_1_mrrs['-ad'][''], rel_change(to_1_mrrs, '-ad', '')))
    #print('--------------------------')
    # performance w.r.t. seen queries (table 5, 7)
    #print('Performance w.r.t. seen/unseen queries')
    #print('\tSeen\t\t\t\tUnseen\t\t')
    #print('%\tOurs\t-RS\t-AD\t%\tOurs\t-RS\t-AD')
    #print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(seen_ratio, seen_mrrs['ours'][''], seen_mrrs['-rs'][''], rel_change(seen_mrrs, '-rs', ''), seen_mrrs['-ad'][''], rel_change(seen_mrrs, '-ad', ''),unseen_ratio, unseen_mrrs['ours'][''], unseen_mrrs['-rs'][''], rel_change(unseen_mrrs, '-rs', ''), unseen_mrrs['-ad'][''], rel_change(unseen_mrrs, '-ad', '')))
    #print()
    ...
    #print('\tTo-many\t\t\t\tTo-one\t\t')
    #print('%\tOurs\t-RS\t-AD\t%\tOurs\t-RS\t-AD')
    #print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(to_m_ratio, to_m_mrrs['ours']['full_kg'], to_m_mrrs['-rs']['full_kg'], rel_change(to_m_mrrs, '-rs', 'full_kg'), to_m_mrrs['-ad']['full_kg'], rel_change(to_m_mrrs, '-ad', 'full_kg'),to_1_ratio, to_1_mrrs['ours']['full_kg'], to_1_mrrs['-rs']['full_kg'], rel_change(to_1_mrrs, '-rs', 'full_kg'), to_1_mrrs['-ad']['full_kg'], rel_change(to_1_mrrs, '-ad', 'full_kg')))
    #print('--------------------------')
    #print('Performance w.r.t. seen/unseen queries')
    #print('\tSeen\t\t\t\tUnseen\t\t')
    #print('%\tOurs\t-RS\t-AD\t%\tOurs\t-RS\t-AD')
    #print('{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})\t{:.1f}\t{:.1f}\t{:.1f} ({:d})\t{:.1f} ({:d})'.format(seen_ratio, seen_mrrs['ours']['full_kg'], seen_mrrs['-rs']['full_kg'], rel_change(seen_mrrs, '-rs', 'full_kg'), seen_mrrs['-ad']['full_kg'], rel_change(seen_mrrs, '-ad', 'full_kg'),unseen_ratio, unseen_mrrs['ours']['full_kg'], unseen_mrrs['-rs']['full_kg'], rel_change(unseen_mrrs, '-rs', 'full_kg'), unseen_mrrs['-ad']['full_kg'], rel_change(unseen_mrrs, '-ad', 'full_kg')))
            ...
            if args.search_random_seed:
                ...
                for i in range(num_runs):
                    ...
                    for key in hits_at_1s:
                        pass
                        #print('{}\t{:.3f}\t{:.3f}\t{:.3f}'.format(key, hits_at_1s[key], hits_at_10s[key], mrrs[key]))
                    #print('------------------------------------------')
                    o_f.write('------------------------------------------\n')
                ...
                #print('* @1: {:.3f}\t@10: {:.3f}\tMRR: {:.3f}'.format(hits_at_1s[best_random_seed],hits_at_10s[best_random_seed],mrrs[best_random_seed]))
                with open(out_log, 'a'):
                ...
                for i, grid_entry in enumerate(list(grid)):
                    ...
                    #print('* @1: {:.3f}\t@10: {:.3f}\tMRR: {:.3f}'.format(hits_at_1s[best_signature],hits_at_10s[best_signature],mrrs[best_signature]))
                    o_f.write('* @1: {:.3f}\t@10: {:.3f}\tMRR: {:.3f}\ns'.format(
...
def run_inference_ginkgo(x1, x2, args=args):
    args.update_run_single_pred(x1,x2)
    run_experiment(args)
